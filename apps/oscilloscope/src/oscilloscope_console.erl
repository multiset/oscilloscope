    ...
    try
        case riak_core:join(NodeStr) of
            ...
            {error, different_ring_sizes} ->
                io:format(
                    "Failed: ~s has a different ring_creation_size~n",
                    [NodeStr]
                ),
                error
        end
...
ringready([]) ->
    try
        case riak_core_status:ringready() of
            {ok, Nodes} ->
                io:format("All nodes agree on the ring ~p~n", [Nodes]);
            {error, {different_owners, N1, N2}} ->
                io:format(
                    "Node ~p and ~p list different partition owners~n",
                    [N1, N2]
                ),
                error;
            {error, {nodes_down, Down}} ->
                io:format(
                    "~p down. All nodes need to be up to check.~n",
                    [Down]
                ),
                error
        end
