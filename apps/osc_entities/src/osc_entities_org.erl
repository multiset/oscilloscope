...
create(Name) when is_binary(Name) ->
    {ok,_,_,[{OwnerID}]} = osc_metadata_sql:named(insert_owner, []),
    {ok,_,_,[{ID}]} = osc_metadata_sql:named(insert_org, [Name, OwnerID]),
...
delete(Org) ->
    ...
    [osc_entities_team:delete(Team) || Team <- teams(Org)],
    {ok, _} = osc_metadata_sql:named(delete_org, [OrgID]),
    {ok, _} = osc_metadata_sql:named(delete_org_members, [OrgID]),
    ok.

-spec teams(#org{}) -> [#team{}].
teams(Org) ->
    lists:foldl(fun(TeamID, Acc) ->
        case osc_entities_team:lookup(Org, TeamID) of
            {ok, Team} ->
                [Team|Acc];
            not_found ->
                lager:error(
...
is_owner(User, Org) ->
    case osc_entities_team:lookup(Org, <<"owners">>) of
        not_found ->
            lager:error(
            ...
            false;
        {ok, Team} ->
            osc_entities_team:is_member(Team, User)
    end.
...
remove_member(Org, User) ->
    ...
    lists:map(fun(TeamID) ->
        case osc_entities_team:lookup(TeamID) of
            not_found ->
                lager:error(
            ...
            {ok, Team} ->
                osc_entities_team:remove_member(Team, User)
        end
    end, TeamIDs),
    ...
    lists:foldl(fun(Team, UserAcc) ->
        {ok, _, UserAcc0} = osc_entities_team:remove_member(Team, UserAcc),
        UserAcc0
    end, User, teams(Org)),
    {ok, NewOrg, NewUser}.
