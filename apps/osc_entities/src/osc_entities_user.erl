...
create(Name, Pass) ->
    ...
    {ok,_,_,[{UserID}]} = osc_metadata_sql:named(
        insert_user,
        [Name, Hash, OwnerID]
    ),
    User = #user{
...
add_email(User, Email) ->
    case lists:member(Email, User#user.emails) of
        ...
        false ->
            ...
            osc_metadata_sql:named(insert_email, [UserID, Email]),
...
delete(User) ->
    dict:fold(fun(OrgID, _TeamIDs, UserAcc) ->
        {ok, Org} = osc_entities_org:lookup(OrgID),
        {ok, NewUser} = osc_entities_org:remove_member(Org, UserAcc),
        NewUser
    end, ok, User#user.orgs),
    true = ets:delete(users, User#user.name),
    true = ets:delete(user_names, User#user.id),
    osc_metadata_sql:named(delete_user, []),
    ok.
...
orgs(User) ->
    ...
    lists:foldl(fun(OrgID, Acc) ->
        case osc_entities_org:lookup(OrgID) of
            not_found ->
                Acc;
            {ok, Org} ->
                [Org|Acc]
