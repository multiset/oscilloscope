...
init([]) ->
    ...
    {ok, _, RawEmails} = osc_metadata_sql:named(select_emails, []),
    Emails = lists:foldl(fun({UserID, Email}, Dict) ->
        dict:append(UserID, Email, Dict)
    end, dict:new(), RawEmails),
    {ok, _, Users} = osc_metadata_sql:named(select_users, []),
    ...
    {ok, _, Orgs} = osc_metadata_sql:named(get_orgs, []),
    ...
    {ok, _, AllTeams} = osc_metadata_sql:named(get_teams, []),
    ...
    {ok, _, UserTeams} = osc_metadata_sql:named(get_all_user_teams, []),
    ...
    {ok, _, OrgTeams} = osc_metadata_sql:named(get_all_org_teams, []),
    populate_ets_records(OrgTeams, org_names, orgs, #org.teams),
    {ok, _, OrgMembers} = osc_metadata_sql:named(get_all_org_members, []),
    populate_ets_records(OrgMembers, org_names, orgs, #org.members),
    {ok, _, TeamMembers0} = osc_metadata_sql:named(get_all_team_members, []),
    TeamMembers = lists:map(fun({OrgID, TeamID, UserID}) ->
        {{OrgID, TeamID}, UserID}
    end, TeamMembers0),
    populate_ets_records(TeamMembers, team_names, teams, #team.members),

    {ok, _, OrgMetrics} = osc_metadata_sql:named(get_org_metrics, []),
    ...
    {ok, _, UserMetrics} = osc_metadata_sql:named(get_user_metrics, []),
