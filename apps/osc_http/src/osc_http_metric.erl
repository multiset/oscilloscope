...
malformed_request(Req, State) ->
    try
        Metric = osc_http_util:get_path_info(metric_name, Req),
        From = osc_http_util:get_qs("from", Req),
        Until = osc_http_util:get_qs("until", Req),
...
to_json(Req, State) ->
    ...
    {ok, Values} = osc:read(Metric, From, Until),
