...
static int apod_scan_judy_double(Pvoid_t judy, int interval, int64_t from, int size, double* arr)
{
  ...
  for (;;) {
    ...
    pvalue = NULL;
    /*
     * Find the next suitable index in the judy. JLN can re-use pindex, because
     * the previous invocation (of either JLI or JLN, depending on which loop
     * iteration we're on) mutated it.
     */
...
int64_t apod_earliest_time(ApodData *data)
{
  ...
  for (;;) {
    ...
    for (i = 0; i < bucket_size; i++) {
      ...
      /*
       * We intentionally leave some truncated values in the array (that is, we
       * only lazily truncate) and use data->floor as a source of truth for what's
       * truly "alive". Therefore t here must be greater than data->floor to be
       * valid.
       */
...
int64_t apod_latest_time(ApodData *data)
{
   ...
   * index (timestamp) of that bucket. Therefore, the last non-null index in the
   * bucket represents the latest datapoint stored in this window.
  */
  bucket = (double*) *pvalue;
  index = (int64_t) pindex;
...
void apod_truncate(ApodData *data, int64_t floor)
{
  ...
  if (data->t < 0 && floor > data->floor) {
    ...
    data->floor = floor;
    /*
     * The bucket that we're looking to truncate is the latest one *before* the
     * bucket containing the provided floor. We can't truncate the bucket with
     * the current floor, because we can't easily guarantee that there aren't
     * points in that bucket *later* than the floor time.
     */
    bucket_width = data->interval * bucket_size;
    truncate_index = floor - bucket_width - floor % bucket_width;
    if (truncate_index >= 0) {
      /*
       * Don't try to truncate negative indices, because that clears the whole
       * array...
       */
