...
divide_array_test() ->
    ?assertEqual({[], []}, osc_cache:divide_array(array:new(), 4)),
    ?assertEqual(
        {[1, 2], [3, 4]},
        osc_cache:divide_array(array:from_list([1, 2, 3, 4]), 2)
    ),
    ?assertEqual(
        {[1, 2, 3, 4], []},
        osc_cache:divide_array(array:from_list([1, 2, 3, 4]), 7)
    ).
...
append_point_test() ->
    ...
    DP1 = osc_cache:append_point(0, 45, DP0),
    ?assertEqual([[45]], array:to_list(DP1)),
    DP2 = osc_cache:append_point(0, 50, DP1),
    ?assertEqual([[50, 45]], array:to_list(DP2)),
    DP3 = osc_cache:append_point(2, 42, DP2),
...
maybe_trim_test() ->
    ?assertEqual(
        ...
        osc_cache:maybe_trim(100, array:from_list([a, b, c, d], null), 10, 20)
    ),
    ?assertEqual(
        {120, array:from_list([c, d], null)},
        osc_cache:maybe_trim(100, array:from_list([a, b, c, d], null), 10, 2)
    ).
