...
persist(Opts) ->
    ...
    case osc_persistence_fsm_sup:spawn([ReqID, self(), Opts]) of
        {ok, Pid} ->
            {ok, ReqID, Pid};
        Else ->
            Else
...
persist(timeout, State) ->
    ...
    {ok, _Persisted} = osc_persistence:persist(
        Resolution,
        ToPersist
    ),
...
vacuum(timeout, State) ->
    ...
    {ok, _Vacuumed} = osc_persistence:vacuum(
        Resolution,
        ToVacuum
    ),
...
cache_fold(Metric, Cache, Resolution, Acc) ->
    {Points, Meta} = osc_cache:cached(Cache, Resolution),
    case Points of
        [] ->
            Acc;
        _ ->
